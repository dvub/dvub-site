...
export const CommentForm = (args: { fileName: string }) => {
	...
	const onFormSubmit = (event: any) => {
		...
		if (
			commentState.name.length <= 1 ||
			commentState.name.length >= maxNameLength
		) {
			return;
		}

		if (
			commentState.comment.length <= 1 ||
			commentState.comment.length >= maxCommentLength
		) {
			return;
		}
	...
	return (
		<Form onSubmit={onFormSubmit} className='border'>
			<p>
				...
				<b>Note:</b> Comments are <b>actively moderated</b>
				{';'} Please be nice (and don{"'"}t spam thx)
			</p>
			...
			<p>
				...
				If you would like to reply to someone, you can try using
				@&lt;username&gt;, or quote some of their comment in yours.
			</p>
		</Form>
	);
};
